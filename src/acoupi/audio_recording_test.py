...
class getRecording_Info(Recording):
    ...
    def __init__(self, id: UUID, path: str, datetime: datetime.datetime, duration: float, samplerate: int):
...
#class PyAudioRecorder(AudioRecorder):
    ...
    def record(self) -> Recording:
        ...
        with tempfile.NamedTemporaryFile(suffix='.wav', delete=False) as temp_audiof:
            ...
            recording = getRecording_Info(id=self.id, path=temp_audio_path, datetime=self.datetime, duration=self.duration, samplerate=self.sample_rate)
            return recording
