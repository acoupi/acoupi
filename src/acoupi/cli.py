...
def show():
    ...
    try: 
        config = system.load_config(system.PROGRAM_CONFIG_FILE, system.CeleryConfig)
...
def get(config_param_name: str):
    ...
    try: 
        ...
        if hasattr(config, config_param_name):
            ...
            click.echo(f"Configuration parameter '{config_param_name}' not found.")
...
def sub(config_param_name, config_param_value):
    ...
    try:
        ...
        if hasattr(config, config_param_name):
            ...
            click.echo(f"Configuration parameter '{config_param_name}' set to '{config_param_value}'.")
        else: 
            click.echo(f"Configuration parameter '{config_param_name}' not found.")
