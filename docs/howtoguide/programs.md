...
- **Send Messages Task**: This periodic task (running every 2 minutes by default, but configurable) checks the message store for pending messages and attempts to deliver them.
    It can optionally limit how many queued messages are sent during each run and choose whether older or newer queued messages are prioritised first.
...
5. [**MessagingConfiguration**][acoupi.programs.templates.MessagingConfig]: This schema encompasses all the necessary settings for configuring the messaging task, including the send interval, heartbeat interval, queue database path, and optional controls for limiting how many queued messages are sent in one run.
...
3. [**`generate_send_messages_task`**][acoupi.tasks.generate_send_messages_task]: This template creates a task responsible for sending pending messages to remote endpoints using the configured messengers (e.g. HTTP or MQTT). It can also restrict how many queued messages are sent during a single run and control whether older or newer messages are selected first.
